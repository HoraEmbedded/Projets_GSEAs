...
P2 — Résolution du conflit Vhv (3 datasheets croisées). SHT4x (datasheet Sensirion v4) : VDD 1,08-3,6 V (aucune contrainte de rail), I_mesure 320 µA typ / durée 6,9 ms haute rép. (confirme budget ✅) ; découplage 100 nF + pull-ups I²C 2×10 kΩ obligatoires (Fig.1). Levier identifié : basse rép. (1,3 ms) inutile ici car mesure = 0,004 % du budget. SX126x : plage ~1,8-3,7 V → accepte 2,5 V → le profil LiFePO4 (CFG=001, HVOUT=2,5 V) convient à la radio → conflit RÉSOLU par simple vérification datasheet (pas de custom mode). Architecture d'alim figée : LVOUT 1,8 V → MCU+capteur ; HVOUT 2,5 V → radio. Point de vigilance : interfaçage SPI MCU 1,8 V ↔ radio 2,5 V (niveaux logiques à vérifier). À vérifier : I_TX du SX1262 à 2,5 V (impact budget). Leçon : un « conflit » se dissout souvent en lisant la vraie plage de spec avant de sur-concevoir.
